...
{
    private Board board;
    //Danh sach cac van co dang dien ra tai cung mot thoi diem
    ...
    public void MatchPiecesOfColor(string color)
    {
        for(int i = 0; i < board.width; i++)
        {
            for(int j = 0; j < board.height; j++)
            {
                ...
                /* Tránh trường hợp phần giải thuật phía sau  
                 * được gọi trước khi viên kẹo mới được sinh ra
                 */
                if(board.allDots[i, j] != null)
                {
                    //Check the tag on that dot
                    /* Kiểm tra giá trị mảng 
                     * 
                     */
